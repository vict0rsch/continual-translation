...
class ContinualModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument(
                "--lambda_R", type=float, default=1, help="weight for rotation",
            )
            parser.add_argument(
                "--lambda_D", type=float, default=1, help="weight for depth",
            )
            parser.add_argument(
                "--lambda_identity",
                type=float,
    ...
    def set_input(self, input):
        ...
        self.depth_A = input["dA" if AtoB else "dB"].to(self.device)  # TODO
        self.depth_B = input["dB" if AtoB else "dA"].to(self.device)  # TODO
    ...
    def backward_G(self):
        ...
        if self.should_compute("depth"):
            ...
            self.loss_G_A_d = self.depthCriterion(self.depth_A_pred, self.depth_A_pred)
            self.loss_G_B_d = self.depthCriterion(self.depth_B_pred, self.depth_B_pred)
